...
function RenteeInfoHolder({
  ...
  const publishedDate = `${date.getFullYear()}-${
    date.getMonth() + 1
  }-${date.getDate()}`;
  ...
  const handleEditClick = () => {
    ...
    setEditRenteeInfo({
      id: rentee?.id,
      thumbnail: import.meta.env.VITE_BEEP_API + rentee?.thumbnailUrl,
      name: rentee?.name,
    ...
    <Container>
      ...
      >
        <IdImgBox>
          ...
          <div id="image">
            <img src={import.meta.env.VITE_BEEP_API + rentee?.thumbnailUrl} />
          </div>
        </IdImgBox>
        {!isBookMode ? (
          <DeviceTextInfo>
